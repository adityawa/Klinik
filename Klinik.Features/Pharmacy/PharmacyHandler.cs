				...
				try
				{
					...
					foreach (var item in request.Data.Medicines)
					{
						...
						if (existEntity == null)
						{
							...
							detail.ProductID = string.IsNullOrEmpty(item.Detail.ProductName) ? null : item.Detail.ProductID;
						...
						{
							existEntity.ProductID = string.IsNullOrEmpty(item.Detail.ProductName) ? null : item.Detail.ProductID;
